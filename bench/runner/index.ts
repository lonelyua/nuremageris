...
function computeStats(timings: number[]) {
  if (timings.length === 0) return { mean: 0, p50: 0, p95: 0, p99: 0, min: 0, max: 0 }
...
async function runCase(
  ...
  while (done < iterations) {
    ...
    const batchTimings = await Promise.all(
      Array.from({ length: batchSize }, async () => {
        ...
        try { await bench.run(adapter, ctx) } catch { errors++ }
  ...
  const memAvgKb  = memSamples.length > 0
  ...
  const cpuAvgMs  = cpuPerReq.length > 0
    ? cpuPerReq.reduce((s, v) => s + v, 0) / cpuPerReq.length / 1000
    : 0
  const cpuPeakMs = cpuPerReq.length > 0
    ? Math.max(...cpuPerReq) / 1000
    : 0
  ...
  process.stdout.write(
    ...
    `  ${dim('heap')} ${fmtKb(result.mem_avg_kb)}/${fmtKb(result.mem_peak_kb)}` +
    `  ${dim('cpu')} ${result.cpu_avg_ms.toFixed(2)}/${result.cpu_peak_ms.toFixed(2)}ms` +
...
function printSummaryTable(results: RunResult[]): void {
  ...
  console.log(
    ...
    dim('    Measures JS/ORM overhead only. Low values (0.1–2 ms) are normal.\n') +
    dim('    avg = mean per request across all steps;  peak = worst single step.\n') +
    dim('    Highlighted green = fastest adapter for that case (lowest mean).')
  )
}
...
function toCsv(results: RunResult[]): string {
  const header = [
    'adapter', 'case', 'iterations', 'concurrency',
    'mean_ms', 'p50_ms', 'p95_ms', 'p99_ms', 'min_ms', 'max_ms',
    'mem_avg_kb', 'mem_peak_kb',
    'cpu_avg_ms', 'cpu_peak_ms',
    'errors',
  ].join(',')

  const rows = results.map(r =>
    [
      r.adapter, r.case, r.iterations, r.concurrency,
      r.mean.toFixed(3),  r.p50.toFixed(3),
      r.p95.toFixed(3),   r.p99.toFixed(3),
      r.min.toFixed(3),   r.max.toFixed(3),
      r.mem_avg_kb, r.mem_peak_kb,
      r.cpu_avg_ms, r.cpu_peak_ms,
      r.errors,
    ].join(',')
  )
...
async function main() {
  ...
  program
    ...
    .option('--warmup <n>',       'Warmup iterations (not measured)',    String(benchConfig.warmup))
    .option('--iterations <n>',   'Measured iterations',                 String(benchConfig.iterations))
    .option('--concurrency <n>',  'Parallel requests per step',          String(benchConfig.concurrency))
  ...
  console.log(`${dim('adapters:')} ${adapterNames.join(', ')}   ${dim('cases:')} ${caseNames.length}`)

  const allResults: RunResult[] = []

  for (const adapterName of adapterNames) {
    console.log(`\n${bold(`── ${adapterName}`)} ${'─'.repeat(Math.max(0, 60 - adapterName.length))}`)

    const adapter = createAdapter(adapterName)

    try {
      for (const caseName of caseNames) {
        const result = await runCase(adapter, caseName, adapterName, warmup, iterations, concurrency)
  ...
  // Persist results first so the table is the last thing on screen
