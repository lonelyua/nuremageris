...
export const cases: BenchCase[] = [
...

  {
    ...
    async run(adapter) {
      return adapter.listUsers({}, { field: 'created_at', dir: 'desc' }, { page: 1, limit: 20 })
    },
  },
...

  {
    ...
    async run(adapter) {
      const ts = Date.now()
      return adapter.insertOneUser({ email: `bench_${ts}@test.com`, name: `Bench ${ts}` })
    },
  },
    ...
    async run(adapter) {
      const data: NewOrderInput = {
        ...
        items: [
          ...
          { productId: 3, quantity: 3, unitPrice: 4.50  },
        ],
        paymentAmount: 59.47,
      }
